...
Gi = find(~isnan(GPS(:,1))); [~,G0] = min(abs(Gi-find(tagon,1))); G1 = GPS(Gi(G0),:);  
proj = projcrs(32610); %WGS 1984
proj.GeographicCRS.Name;
[x1,y1] = projfwd(proj,G1(1),G1(2)); 
Lats = nan(size(tagon)); Longs = Lats; startx = 0; starty = 0;

[Lats,Longs] = projinv(proj,geoPtrack(tagon,1)+x1,geoPtrack(tagon,2)+y1);
