...
% Writes data onto video as well as audio from separate files.  Cuts the video into
...
cd(fileloc); 
[afiles,audiofileloc] = uigetfile('*.wav','select audio file (or files) from the deployment (press cancel if no audio))','multiselect','on');
if isempty(afiles); noaud = true; 
else
    if ischar(afiles); afiles = {afiles}; end
    noaud = false;
end

if noaud; disp(['No audio will be written on movie files']);
else
    audStarts = nan(size(afiles));
    audEnds = audStarts;
    for ii = 1:length(afiles)
        afile = afiles{ii};
        try dashes = regexp(afile,'-');
            try audStart = datenum(afile(dashes(end-1)-8:regexp(afile,'.wav')-1),'yyyymmdd-HHMMSS-fff');
        ...
        catch; disp(['Audio File: ' afile])
            a = input('Audio Start Time could not be read from file name, input as datevec ([yyyy mm dd HH MM SS.fff]):');
            audStart = datenum(a);
        end
        disp(['Audio file ' num2str(ii) ' starts at ' datestr(audStart,'dd-mmm-yyyy HH:MM:SS.fff')])
        audioInfo = audioinfo([audiofileloc afile]);
        audEnd = audStart + audioInfo.Duration/24/60/60;
        disp(['Audio file ' num2str(ii) ' ends at ' datestr(audEnd,'dd-mmm-yyyy HH:MM:SS.fff')]);
        audStarts(ii) = audStart;
        audEnds(ii) = audEnd;
    end
end
try cd('////GOLDTERA1//lab//TEMP//prhs'); catch; end %cd(
...
%                 kk = 0;
                ...
                while length(vid.frames)<150 && ET+.01<TD && ET < 100*60*60; % got rid of 30+60 since ET will always be shorter
                    ...
                    j = j+1;
                    ET = min([dur*j+ST0,endtime, 100*60*60]); % endtime
                    [vid,~] = mmread([fileloc filename{n}], [],[ST ET],false);
...
%             scrn = get(0,'screensize');
        ...
        for i = 1:length(vid.frames)
            ...
            if i == 1
                k = find(audStarts<istamp,1,'last');
                audStart = audStarts(k);
                audEnd = audEnds(k);
                afile = afiles{k};
            end
            if i == 1 && istamp >= audStart  && istamp<=audEnd
                atimes(1) = (istamp-audStart)*24*60*60;
            elseif i == length(vid.frames) && istamp >= audStart % && istamp<=audEnd
                atimes(2) = (istamp-audStart)*24*60*60;
            end
            [~,b] = min(abs(DN-istamp));
...
%         if n>1 || j>5
        if isinf(dur); mmwrite([filedest 'partial//' filename{dirN}(1:end-4) '//' filename{n}(1:end-4) 'SxS.avi'],vid,aud,conf); else
            ...
            if noaud; mmwrite([filedest 'partial//' prhfile(1:regexp(prhfile,' ')-1) '//' filename{dirN}(1:end-4) '//' es prhfile(1:regexp(prhfile,' ')-1) ' (' vN ')' tail '.avi'],vid,conf);
                  ...
                  [~,aud]= mmread([audiofileloc afile], [],atimes,true);  %just audio
                  if atimes(2)>aud.totalDuration & k<length(afiles)
                      atimes(1) = 0; atimes(2) = vid.times(end)-vid.times(1)-(aud.totalDuration-atimes(1));
                      [~,aud2]= mmread([audiofileloc afiles{k+1}], [],atimes,true);  %just audio
                      aud.data = [aud.data; aud2.data];
                      disp(['Transition between audiofiles ' num2str(k) ' & ' num2str(k+1)])
                  end
        
                  % if ~ismember(100,factor(aud.rate)) % if audio is a funny rate, resulting videos are hard to read by adobe
                  %   newAfs = 24000;
                  %     X = resample(aud.data,(1:length(aud.data))/aud.rate,newAfs);
                  %     aud.data = X; aud.rate = newAfs;
                  % end
                  if aud.rate >48000; df = factor(aud.rate); %df = ceil(aud.rate/48000); if abs(aud.rate/df-round(aud.rate/df))>.01; error('audio rate is too big for mmread and suggested decimation factor does not divide evenly into audio rate'); end
                      df = df(find(aud.rate./df<=48000,1));
                      aud.data = decdc(aud.data,df); aud.rate = aud.rate/df;
                      % aud.data = resample(aud.data,48000,aud.rate);
                      % aud.rate = 48000;
                      warning(['Audio rate decimated to ' num2str(aud.rate) ' to work with mmwrite function']);
                  end
                 aud.times = aud.times-aud.times(1)+vid.times(1);
