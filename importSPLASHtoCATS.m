function [data, Adata, Atime] = importSPLASHtoCATS(fileloc, filename)
%
...
% Stanford University
%
% This script imports data from the Wildlife Computers SPLASH tag to the new 
% CATS data processing standards (11/2020), which includes the addition of 
% Adata, Atime and Hzs to the output of the script
%
% format:
% importSPLASHtoCATS(fileloc, filename);
% importSPLASHtoCATS();
% [data, Adata, Atime] = importSPLASHtoCATS(...);
%
% filename is the Archive CSV file outputted by DAP Processor
%
% NOTE: Raw data from SPLASH is in GMT; No conversion to Local time takes place in this script

% nargin = 0; %uncomment this line if you are running from the code (not as a function)
if nargin <2;
    [filename,fileloc] = uigetfile('*.csv','Select the Wildlife Computers SPLASH Archive CSV file (exported from DAP Processor).');
end
...
%% 1. Import Section
        ...
        if vers<2020
            ...
            data = readtable([fileloc filename],'Format','%s%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%s%s%u%s');
        ...
        headers(~cellfun(@isempty,strfind(headers,'intM'))) = {'Comp1' 'Comp2' 'Comp3'};
